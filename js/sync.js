...
// All data writes go through Index which handles Firebase + local in parallel

const Sync = {
  _listeners: [],   // Firebase real-time unsubscribe functions
  _notifCount: 0,

  async getSheetsUrl() { return IndexDB.getSetting('sheets_url', ''); },
  ...
  async queueSubmission(submission) {
    await Index.queuePut({ recordId: submission.recordId, ...submission });
  },
  ...
  async syncSubmission(submission) {
    ...
    try {
      await this.pushToSheets({
        ...
        duration_mins:  submission.durationMins != null ? submission.durationMins : '',
      ...
      const sub = await IndexDB.get('submissions', submission.recordId);
      if (sub) { sub.synced = true; await IndexDB.put('submissions', sub); }
      await Index.queueDelete(submission.recordId).catch(() => {});
      return true;
    } catch(e) {
