...
window.adminFilterTickets = async (filter) => {
  document.querySelectorAll('#admin-content .chip').forEach(c => {
    c.classList.toggle('selected', c.textContent.toLowerCase().startsWith(filter === 'all' ? 'all' : filter.replace('_',' ')));
  });
  const container = document.getElementById('ticket-list-container');
  if (container) await Tickets.renderAdminTicketList(container, filter === 'emergency' ? null : filter, filter === 'emergency');
};
