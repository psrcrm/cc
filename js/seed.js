...
async function seedLocalData() {
  const existing = await IndexDB.getAll('workers');
  ...
  for (const w of workers) await IndexDB.put('workers', w);
  ...
  for (const t of templates) await IndexDB.put('templates', t);
  ...
  for (let d = -4; d <= 0; d++) {
    ...
    for (let i=0; i<taskDefs.length; i++) {
      ...
      await IndexDB.put('tasks', { id:`${def.workerId}-${def.templateId}-${dateStr}`, workerId:def.workerId, templateId:def.templateId, templateName:tpl.name, templateIcon:tpl.icon, category:tpl.category, date:dateStr, dueTime:def.time, status, communityId:'COMM-001', assignedAt:new Date().toISOString() });
    }
  }
  ...
  for (const t of demoTickets) await IndexDB.put('tickets', t);

  await IndexDB.setSetting('seeded', true);
