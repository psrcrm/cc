...
const Auth = {
  ...
  async login(mobile, pin) {
    ...
    if (Data._useFirebase) {
      try {
        ...
        if (!snap.empty) {
          ...
          await IndexDB.put('workers', worker);
        }
      } catch(e) {
    ...
    // 2. Always fall back to IndexDB (offline / first load / Firestore not enabled yet)
    if (!worker) {
      const all = await IndexDB.getAll('workers');
    ...
    await IndexDB.setSetting('lastUserId',   worker.id);
    await IndexDB.setSetting('lastUserData', JSON.stringify(worker));
  ...
  async autoLogin() {
    const id  = await IndexDB.getSetting('lastUserId',   null);
    const raw = await IndexDB.getSetting('lastUserData', null);
      ...
      if (Data._useFirebase) {
        FB.db.collection('workers').doc(id).get().then(doc => {
          if (doc.exists) {
            ...
            IndexDB.put('workers', fresh);
            IndexDB.setSetting('lastUserData', JSON.stringify(fresh));
          }
        }).catch(()=>{});
      }
      return cached;
  ...
  async _getWorker(id) {
    ...
    return w || await IndexDB.get('workers', id);
  },
